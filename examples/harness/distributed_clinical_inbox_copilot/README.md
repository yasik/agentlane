...
5. resolved host:port, worker id, and process id for each runtime node in the UI
...
8. optional multi-process worker execution using the same runtime host
...
By default the demo starts all runtime workers inside the controller process.
Use `--multiprocess` to launch the specialist and aggregator workers as separate
Python processes:
...
For a model-free runtime smoke test that still exercises distributed fan-out and
fan-in:
...
The live dashboard shows the host address, each worker host:port, each worker
id, and each process id. Specialist work is published through the host using
`REVIEW_TOPIC_TYPE`; each specialist publishes a finding to `RESULT_TOPIC_TYPE`;
the stateful aggregator publishes the final `AggregatedClinicalReview` back to
the controller process after all findings arrive.
