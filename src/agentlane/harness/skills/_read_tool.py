...
activated skill with a listed resource path and passes the result here; the tool
reads that root-relative path from the first root that has it.
"""
...
class _ToolArgs(BaseModel):
    ...
    path: str = Field(
        description=(
            "Root-relative path of a skill resource: the skill directory shown "
            "when the skill activated, joined with a listed resource path."
        )
    )
...
def filesystem_read_tool(
    ...
    Args:
        ...
        roots: Roots to search, in precedence order (an earlier root wins). Match
            the roots a `FilesystemSkillLoader` discovers from so the model can
            read any resource it was shown.
...
async def _read_resource(
    ...
    *,
    ...
    """Read one resource across the roots and render a plain-text tool result."""
    data: bytes | None = None
    for root in roots:
        try:
            data = await filesystem.read_bytes(root, path)
            break
