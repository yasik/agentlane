...
class RunEventStream(AsyncIterator[RunEvent]):
    ...
    async def __anext__(self) -> RunEvent:
        """Return the next high-level run event."""
        item = await self._queue.get()
        if item is _RUN_EVENT_STREAM_END:
    ...
    async def aclose(self) -> None:
        ...
        self._queue.put_nowait(_RUN_EVENT_STREAM_END)
